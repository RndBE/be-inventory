...
class LaporanProyekExport implements FromArray, WithHeadings, ShouldAutoSize, WithStyles, WithTitle
{
    protected $projek_id;
    ...
    public function array(): array
    {
        ...
        $laporanProyek = LaporanProyek::where('projek_id', $this->projek_id)->get();
        $totalQtyTambahan = 0;
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $bahanRusak = BahanRusak::with('bahanRusakDetails.dataBahan')->where('projek_id', $this->projek_id)->get();
        ...
        $totalSubTotal = $projek->projekDetails->sum(fn($detail) => $detail->sub_total);
        ...
        $sheet->setCellValue("F{$startRowBiaya}", "Biaya Bahan/Produk:");
        $sheet->setCellValue("G{$startRowBiaya}", "Rp. " . number_format($biayaBahanProduk, 2, ',', '.'));

        $sheet->setCellValue("F" . ($startRowBiaya + 1), "Biaya Tambahan:");
        $sheet->setCellValue("G" . ($startRowBiaya + 1), "Rp. " . number_format($biayaTambahan, 2, ',', '.'));

        $sheet->setCellValue("F" . ($startRowBiaya + 2), "Bahan Rusak:");
        $sheet->setCellValue("G" . ($startRowBiaya + 2), "Rp. " . number_format($biayaBahanRusak, 2, ',', '.'));

        $sheet->setCellValue("F" . ($startRowBiaya + 3), "Total Pengeluaran:");
        $sheet->setCellValue("G" . ($startRowBiaya + 3), "Rp. " . number_format($totalPengeluaran, 2, ',', '.'));

        // Buat format angka di kolom biaya agar tetap dalam format mata uang
        for ($row = $startRowBiaya; $row <= $startRowBiaya + 3; $row++) {
            $sheet->getStyle("G{$row}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
            $sheet->getStyle("G{$row}")->getNumberFormat()->setFormatCode('[$-421] #,##0.00');
        }

        // Buat teks di kolom F bold
        $sheet->getStyle("F{$startRowBiaya}:F" . ($startRowBiaya + 3))->getFont()->setBold(true);
        $sheet->getStyle("F{$startRowBiaya}:F" . ($startRowBiaya + 3))->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
