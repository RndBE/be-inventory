...
class ProdukJadisExport implements FromArray, WithHeadings, WithStyles, WithEvents, WithCharts
{
    ...
    protected $stokAkhirColIndex = null;
    ...
    public function array(): array
    {
        ...
        $tglBlnHeaders[] = 'Asal Flow';
        $tglBlnHeaders[] = 'Tujuan Flow Terakhir';
        $tglBlnHeaders[] = 'Kode Tujuan Terakhir';
        $tglBlnHeaders[] = 'Status Flow';
        ...
        foreach ($allDetails as $entry) {
            ...
            $this->stokAkhirColIndex = count($row);
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        foreach (['Asal Flow', 'Tujuan Flow Terakhir', 'Kode Tujuan Terakhir', 'Status Flow'] as $flowHeader) {
            $flowColumnLetter = $this->getColumnLetter($colIndex);
            $sheet->mergeCells("{$flowColumnLetter}3:{$flowColumnLetter}4");
            $sheet->setCellValue("{$flowColumnLetter}3", $flowHeader);
            $sheet->getStyle("{$flowColumnLetter}3:{$flowColumnLetter}4")->getFont()->setBold(true);
            $sheet->getStyle("{$flowColumnLetter}3:{$flowColumnLetter}4")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $colIndex++;
        }

        $lastColumnLetter = $this->getColumnLetter($colIndex - 1);
        ...
        if (!is_int($this->stokAkhirColIndex)) {
            $this->produkSummary = [];
            return;
        }
