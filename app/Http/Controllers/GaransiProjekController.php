...
use App\Exports\GaransiProjekExport;
...
class GaransiProjekController extends Controller
{

    public function __construct()
    {
        $this->middleware('permission:lihat-garansi-projek', ['only' => ['index']]);
        $this->middleware('permission:selesai-garansi-projek', ['only' => ['updateStatus']]);
        $this->middleware('permission:tambah-garansi-projek', ['only' => ['create','store']]);
        $this->middleware('permission:edit-garansi-projek', ['only' => ['update','edit']]);
        $this->middleware('permission:hapus-garansi-projek', ['only' => ['destroy']]);
    }

    public function info($id)
    {
        $garansiProjek = GaransiProjek::with([
            'dataBahan',
            'bahanKeluar',
        ...
        $allKode = [];
        foreach ($garansiProjek->bahanKeluar as $bk) {
        ...
        return view('pages.garansi-projek.info', compact('garansiProjek', 'lookupTransaksi'));
    }

    public function export($projek_id)
    {
        $projek = GaransiProjek::findOrFail($projek_id);
        $fileName = 'HPP_GaransiProject_' . $projek->nama_projek . '_be-inventory.xlsx';
        return Excel::download(new GaransiProjekExport($projek_id), $fileName);
    }
    ...
    {
        return view('pages.garansi-projek.index');
    }
    ...
    public function create()
    {
        ...
        $usedKontrakIds = GaransiProjek::pluck('kontrak_id')->toArray();
        ...
        return view('pages.garansi-projek.create', compact('units', 'produkProduksi', 'kontraks', 'usedKontrakIds'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $validator = Validator::make([
                ...
                'mulai_garansi' => $request->mulai_garansi,
                'keterangan' => $request->keterangan,
                'cartItems' => $cartItems
            ], [
                'kontrak_id' => 'required',
                'mulai_garansi' => 'required',
                'keterangan' => 'required',
            ...
            $lastTransactionProjek = GaransiProjek::orderByRaw('CAST(SUBSTRING(kode_garansi, 7) AS UNSIGNED) DESC')->first();
            $new_transaction_number_produksi = ($lastTransactionProjek ? intval(substr($lastTransactionProjek->kode_garansi, 6)) : 0) + 1;
            $kode_garansi = 'PRJ - ' . str_pad($new_transaction_number_produksi, 5, '0', STR_PAD_LEFT) . ' G';
            ...
            $projek = GaransiProjek::create([
                'kode_garansi' => $kode_garansi,
                'kontrak_id' => $request->kontrak_id,
                'pengaju' => $user->name,
                'keterangan' => $request->keterangan,
                'mulai_garansi' => $request->mulai_garansi,
    ...
    public function edit(string $id)
    {
        ...
        $projek = GaransiProjek::with([
            'garansiProjekDetails.dataBahan',
            'garansiProjekDetails.dataProduk', // Tambahkan ini untuk memuat produk
            'bahanKeluar'
        ])->findOrFail($id);
        // dd($projek->garansiProjekDetails);


        // Ambil bahan yang ada di garansiProjekDetails
        $existingBahanIds = $projek->garansiProjekDetails->pluck('dataBahan.id')->toArray();

        $isComplete = true;
        if ($projek->garansiProjekDetails && count($projek->garansiProjekDetails) > 0) {
            foreach ($projek->garansiProjekDetails as $detail) {
        ...
        return view('pages.garansi-projek.edit', [
            'projekId' => $id,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            // dd($request->all());
            $garansiProjekDetails = json_decode($request->garansiProjekDetails, true) ?? [];
            $bahanRusak = json_decode($request->bahanRusak, true) ?? [];
            $bahanRetur = json_decode($request->bahanRetur, true) ?? [];
            $projek = GaransiProjek::findOrFail($id);
            ...
            foreach ($garansiProjekDetails as $item) {
                $bahan_id = $item['bahan_id'] ?? null;
                $produk_id = $item['produk_id'] ?? null;
                $serial_number = $item['serial_number'] ?? null;
            ...
            LogHelper::success('Berhasil Mengubah Detail Garansi Projek!');
            return redirect()->back()->with('success', 'Garansi projek berhasil diperbarui!');
        } catch (\Exception $e) {
            LogHelper::error($e->getMessage());
    ...
    public function updateStatus(Request $request, $id)
    {
        try{
            ...
            $projek->selesai_garansi= now()->setTimezone('Asia/Jakarta')->format('Y-m-d H:i:s');
            $projek->save();
            LogHelper::success('Berhasil menyelesaikan garansi projek!');
            return redirect()->back()->with('error', 'Garansi projek tidak bisa diupdate ke selesai.');
        }catch(Throwable $e){
            LogHelper::error($e->getMessage());
    ...
    {
        try{
            $projek = GaransiProjek::find($id);
            if (!$projek) {
                return redirect()->back()->with('gagal', 'Garansi projek tidak ditemukan.');
            }
            $projek->delete();
            LogHelper::success('Garansi projek berhasil dihapus!');
            return redirect()->back()->with('success', 'Garansi projek berhasil dihapus!');
        }catch(Throwable $e){
            LogHelper::error($e->getMessage());
            return view('pages.utility.404');
        }
