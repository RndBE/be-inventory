...
class PembelianBahanController extends Controller
{
    ...
    public function updateApprovalDirektur(Request $request, int $id)
    {
        ...
        try {
            DB::beginTransaction();
            // Ambil data pembelian bahan dengan relasi yang dibutuhkan
            $data = PembelianBahan::with([
                'dataUser.atasanLevel1',
                'dataUser.atasanLevel2',
                'dataUser.atasanLevel3',
            ...
            // Periksa apakah status admin manager sudah disetujui
            ...
            // Update status direktur
            $data->status = $validated['status'];
            // Jika direktur menyetujui, simpan tgl_keluar dan proses data pengajuan
            if ($validated['status'] === 'Disetujui') {
                ...
                foreach ($details as $detail) {
                    // Pastikan details berupa array valid
                    ...
                    if ($data->pengajuan_id) {
                        ...
                        $groupedDetails = []; // Kumpulkan data transaksi berdasarkan unit_price
                        foreach ($transactionDetails as $transaksiDetail) {
                        ...
                        if ($existingDetail) {
                            ...
                            $existingDetail->details = json_encode(array_values($mergedDetails)); // Simpan array numerik
                            $existingDetail->save();
                        } else {
                            ...
                            PengajuanDetails::create([
                                ...
                                'jml_bahan' => $detail->jml_bahan,
                                'used_materials' => $detail->jml_bahan,
                                'details' => json_encode(array_values($groupedDetails)), // Simpan array numerik
