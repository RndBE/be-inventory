...
class StockOpnameController extends Controller
{
        ...
        try {
            ...
            foreach ($stockOpname->stockOpnameDetails as $detail) {
                ...
                if ($lastPurchaseDetail) {
                    $qtyTerpakai = $selisih;
                    $qtyTerpakaiAudit = $selisih_audit;

                    $harga = $qtyTerpakai * $lastPurchaseDetail->unit_price;
                    $hargaAudit = $qtyTerpakaiAudit * $lastPurchaseDetail->unit_price;

                    $alokasiHarga[] = "{$qtyTerpakai} x " . number_format($lastPurchaseDetail->unit_price, 0, ',', '.');
                    $alokasiHargaAudit[] = "{$qtyTerpakaiAudit} x " . number_format($lastPurchaseDetail->unit_price, 0, ',', '.');
                    $totalHarga += $harga;
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make([
            ...
            'cartItems.*.tersedia_sistem' => 'nullable',
            'cartItems.*.tersedia_fisik' => 'nullable',
            'cartItems.*.selisih' => 'nullable',
