...
use Carbon\Carbon;
class PengajuanPembelianTable extends Component
{
    use WithPagination;
    public $search = "";
    public $perPage = 25;
    ...
    public function render()
    {
        ...
        $pembelian_bahan->where(function ($pembelian_bahan) {
            $pembelian_bahan->where('tgl_keluar', 'like', '%' . $this->search . '%')
                ...
                ->orWhereHas('dataUser', function ($pembelian_bahan) {
                    $pembelian_bahan->where('name', 'like', '%' . $this->search . '%');
                })
                ->orWhereHas('dataPengajuan', function ($pembelian_bahan) {
                    $pembelian_bahan->where('kode_pengajuan', 'like', '%' . $this->search . '%');
                })
                ->orWhere('kode_transaksi', 'like', '%' . $this->search . '%');
        })
            ->when($this->selectedTab  === 'pengajuan', function ($pembelian_bahan) {
                return $pembelian_bahan->where('status_pembelian', 'Pengajuan');
            })
            ->when($this->selectedTab  === 'diproses', function ($pembelian_bahan) {
                return $pembelian_bahan->where('status_pembelian', 'Diproses');
            })
            ->when($this->selectedTab  === 'selesai', function ($pembelian_bahan) {
                return $pembelian_bahan->where('status_pembelian', 'Selesai');
            })
            ->when($this->filter === 'Ditolak', function ($pembelian_bahan) {
                return $pembelian_bahan->where('status', 'Ditolak');
            })
            ->when($this->filter === 'Disetujui', function ($pembelian_bahan) {
                return $pembelian_bahan->where('status', 'Disetujui');
            })
            ->when($this->filter === 'Belum disetujui', function ($pembelian_bahan) {
                return $pembelian_bahan->where('status', 'Belum disetujui');
            });

        // Paginate hasil pembelian_bahan
        $pembelian_bahans = $pembelian_bahan->paginate($this->perPage);


        // Return ke view
        return view('livewire.pengajuan-pembelian-table', [
