                        ...
                        <tr
                            ...
                            <td class="px-6 py-4">
                                <div class="text-slate-800 dark:text-slate-100">
                                    {{ $bahan_keluars->firstItem() + $index }}</div>
                            </td>
                            <th scope="row"
                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <button class="text-blue-600 hover:underline" type="button"
                                    wire:click="showBahanKeluar({{ $bahan_keluar->id }})">
                            ...
                            <td class="px-6 py-4">Rp
                                {{ number_format($bahan_keluar->bahanKeluarDetails->sum('sub_total'), 2, ',', '.') }}
                            </td>
                            <td class="px-6 py-4">
                                @if ($bahan_keluar->status_pengambilan == 'Belum Diambil')
                                    ...
                                    <span
                                        class="inline-flex items-center whitespace-nowrap bg-blue-100 px-2.5 py-0.5 rounded-full  text-blue-800 text-xs font-medium dark:bg-gray-700 dark:text-blue-400 border border-blue-400">{{ $bahan_keluar->status_pengambilan }}</span>
                                @elseif($bahan_keluar->status_pengambilan == 'Sudah Diambil')
                                    ...
                                    <span
                                        class="inline-flex items-center whitespace-nowrap bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-green-400 border border-green-100">{{ $bahan_keluar->status_pengambilan }}</span>
                                @else
                                    <span
                                        class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-red-400 border border-red-100">{{ $bahan_keluar->status_pengambilan ?? 'Null' }}</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                @if ($bahan_keluar->status_leader == 'Belum disetujui')
                                    ...
                                    <span
                                        class="inline-flex items-center whitespace-nowrap bg-blue-100 me-2 px-2.5 py-0.5 rounded-full text-blue-800 text-xs font-medium dark:bg-gray-700 dark:text-blue-400 border border-blue-400">{{ $bahan_keluar->status_leader }}</span>
                                @elseif($bahan_keluar->status_leader == 'Disetujui')
                                    <span
                                        class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-green-400 border border-green-100">{{ $bahan_keluar->status_leader }}</span>
                                @else
                                    <span
                            ...
                            <td class="px-6 py-4">
                                @if ($bahan_keluar->status == 'Belum disetujui')
                                    ...
                                    <span
                                        class="inline-flex items-center whitespace-nowrap bg-blue-100 me-2 px-2.5 py-0.5 rounded-full text-blue-800 text-xs font-medium dark:bg-gray-700 dark:text-blue-400 border border-blue-400">{{ $bahan_keluar->status }}</span>
                                @elseif($bahan_keluar->status == 'Disetujui')
                                    <span
                                        class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-gray-700 dark:text-green-400 border border-green-100">{{ $bahan_keluar->status }}</span>
                                @else
                                    <span
                            ...
                            <td class="px-6 py-4">
                                <div class="row flex space-x-2">
                                    @if ($bahan_keluar->status === 'Disetujui')
                                        <a href="{{ route('bahan-keluars.downloadPdf', $bahan_keluar->id) }}"
                                            target="__blank"
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                ...
                                                class="w-[16px] h-[16px] icon icon-tabler icons-tabler-outline icon-tabler-file-type-pdf">
                                    ...
                                    @endif
                                    @if (
                                        $bahan_keluar->status !== 'Disetujui' &&
                                            $bahan_keluar->status !== 'Ditolak' &&
                                            $bahan_keluar->status !== 'Pengajuan Ulang' &&
                                            $bahan_keluar->status_leader === 'Disetujui')
                                    ...
                                    @if ($bahan_keluar->status_pengambilan === 'Belum Diambil' && $bahan_keluar->status === 'Disetujui')
                                        @can('edit-pengambilan')
                                            <button wire:click="editPengambilanBahanKeluar({{ $bahan_keluar->id }})"
                                                class="rounded-md border border-slate-300 py-1 px-2 text-center text-xs transition-all shadow-sm hover:shadow-lg text-slate-600 hover:text-white hover:bg-yellow-600 hover:border-yellow-600 focus:text-white focus:bg-yellow-600 focus:border-yellow-600 active:border-yellow-600 active:text-white active:bg-yellow-600 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                                                type="button">
                                    ...
                                    @if ($bahan_keluar->status === 'Disetujui')
                                        @can('edit-bahan-keluar')
                                            <form method="POST"
                                                action="{{ route('send.siap-ambil', $bahan_keluar->id) }}">
                                                @csrf
                                                <button
                                                    class="rounded-md border border-slate-300 py-1 px-2 text-center text-xs transition-all shadow-sm hover:shadow-lg text-slate-600 hover:text-white hover:bg-yellow-600 hover:border-yellow-600 focus:text-white focus:bg-yellow-600 focus:border-yellow-600 active:border-yellow-600 active:text-white active:bg-yellow-600 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                                                    type="submit">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                        ...
                                                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                                        aria-hidden="true"
                                                        class="w-[16px] h-[16px] icon icon-tabler icons-tabler-outline icon-tabler-send text-gray-800 dark:text-white">
                                    ...
                                    @if ($bahan_keluar->status !== 'Disetujui' && $bahan_keluar->status !== 'Ditolak')
                                        {{-- @can('edit-approve-leader')
                                            ...
                                            @endif
                                        @endcan --}}
                                        {{-- Button hanya muncul untuk pemegang slot approval awal. Dipakai
                                             satu sumber dengan pengecekan 403 di BahanKeluarController. --}}
                                        @php
                                            $loginUser = Auth::user();
                                            $approverAwal = $bahan_keluar->approverLeader();
                                            $bolehApproveAwal = $approverAwal && (int) $approverAwal->id === (int) $loginUser->id;
                                        @endphp
                                        ...
                                        @if ($bolehApproveAwal)
                                            @if ($bahan_keluar->status_leader !== 'Disetujui' && $bahan_keluar->status_leader !== 'Ditolak')
                                                <button wire:click="editBahanKeluar({{ $bahan_keluar->id }})"
                                                    class="rounded-md border border-slate-300 py-1 px-2 text-center text-xs transition-all shadow-sm hover:shadow-lg text-slate-600 hover:text-white hover:bg-yellow-600 hover:border-yellow-600 focus:text-white focus:bg-yellow-600 focus:border-yellow-600 active:border-yellow-600 active:text-white active:bg-yellow-600 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
                                                    Approve {{ $bahan_keluar->approvalAwalRole() }}
                                                </button>
                                            @endif
                                        @endif
                                        ...
                                        @can('hapus-bahan-keluar')
                                            @if ($bahan_keluar->status !== 'Disetujui')
                                                <button wire:click="deleteBahanKeluars({{ $bahan_keluar->id }})"
                                                    class="rounded-md border border-slate-300 py-1 px-2 text-center text-xs transition-all shadow-sm hover:shadow-lg text-slate-600 hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white focus:bg-red-600 focus:border-red-600 active:border-red-600 active:text-white active:bg-red-600 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                                                    type="button">
                                                    <svg class="w-[16px] h-[16px] text-gray-800 dark:text-white"
