    ...
    <div class="border-b border-gray-900/10 pb-2 mb-2">
        <div class="grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2">
            ...
            <div class="flex items-center">
                ...
                <select wire:model="jenisPengajuan" name="jenis_pengajuan" id="jenis_pengajuan"
                    ...
                    <tbody>
                        @foreach($cartItems as $item)
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                                    <div class="flex justify-right items-right">
                                        <input value="{{ old('penanggungjawabaset.'.$item->id, $penanggungjawabaset[$item->id] ?? 0) }}"
                                            type="text"
                                            wire:model="penanggungjawabaset.{{ $item->id }}"
                                            wire:keyup="updateQuantity({{ $item->id }})"
                                            class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required/>
                                    </div>
                                </td>
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                                    <div class="flex justify-right items-right">
                                        <textarea
                                            wire:model="alasan.{{ $item->id }}"
                                            wire:keyup="updateQuantity({{ $item->id }})"
                                            ...
                                            required>{{ old('alasan.'.$item->id, $alasan[$item->id] ?? '') }}</textarea>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="#" class="font-medium text-red-600 dark:text-red-500 hover:underline" wire:click.prevent="removeItem({{ $item->id }})"><svg class="w-6 h-6 text-red-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
