...
<div>
    ...
    class="text-right bg-gray-50 w-32 text-gray-900 text-sm rounded-lg focus:outline-none focus:ring-transparent focus:border-none block px-2.5 py-1 dark:bg-gray-700 dark:border-transparent dark:text-white"
    placeholder="0"
    readonly
    required
/>
            ...
            <td class="px-6 py-4">
                <div class="flex justify-center items-center">
                    @if($editingItemId === $item->id)
                        ...
                        />
                    @else
                        <span class="cursor-pointer" wire:click="editItem({{ $item->id }})">{{ $tersedia_fisik[$item->id] ?? 0 }}</span>
                    @endif
                </div>
            </td>
