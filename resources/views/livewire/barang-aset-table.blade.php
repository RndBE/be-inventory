<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-2">
        ...
        <div class="mb-4 sm:mb-0">
            <h6 class="text-2xl text-gray-800 dark:text-gray-100 font-bold">Barang Aset</h6>
        </div>
        ...
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            <ul class="flex flex-wrap -m-1">
                ...
                <li class="m-1">
                    @can('tambah-barang')
                        @include('pages.barang_aset.create')
                    @endcan
                </li>
    ...
    <div class="relative overflow-x-auto pt-2">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @forelse($barang_asets as $index => $row)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-6 py-4"><div class="text-slate-800 dark:text-slate-100">{{ $barang_asets->firstItem() + $index }}</div></td>
                            <td class="px-6 py-3">{{ $row->kode_barang }}</td>
                            <td class="px-6 py-3">{{ $row->nama_barang }}</td>
                            <td class="px-6 py-3">{{ $row->jenisBahan->nama ?? 'N/A' }}</td>
                            <td class="px-6 py-3">{{ $row->dataUnit->nama ?? 'N/A' }}</td>
                            <td class="px-6 py-4">
                                <div class="row flex space-x-2">
                                    <!-- Detail Button -->
                                    <!-- Edit Button -->
                                    @can('edit-barang')
                                        <a wire:click="editBarang({{$row->id}})" class="rounded-md border border-slate-300 py-1 px-2 text-center text-xs transition-all shadow-sm hover:shadow-lg text-slate-600 hover:text-white hover:bg-yellow-600 hover:border-yellow-600 focus:text-white focus:bg-yellow-600 focus:border-yellow-600 active:border-yellow-600 active:text-white active:bg-yellow-600 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
