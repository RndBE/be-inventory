            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-right" style="width: 25%;">Details</th>
                        <th scope="col" class="px-6 py-3 text-right" style="width: 15%;">Qty</th>
                        <th scope="col" class="px-6 py-3 text-right" style="width: 25%;">Sub Total</th>
                        <th scope="col" class="px-6 py-3 text-right" style="width: 5%;">Action</th>
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-col space-y-2">
                                    @if(isset($this->details[$item->id]))
                                        @foreach($this->details[$item->id] as $detail)
                                            <div class="flex justify-end items-center">
                                                <!-- Tampilkan qty dan harga per unit -->
                                                <p>{{ $detail['qty'] }} x Rp. {{ number_format($detail['unit_price'], 0, ',', '.') }}</p>

                                                <!-- Tombol untuk decrease quantity -->
                                                <button wire:click="decreaseQuantityPerPrice({{ $item->id }}, {{ $detail['unit_price'] }})"
                                                    ...
                                                    type="button" {{ $status === 'Selesai' ? 'disabled' : '' }}>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <input value="{{ old('qty.'.$item->id, $qty[$item->id] ?? 0) }}"
                                        ...
                                        placeholder="0" min="0" required {{ $status === 'Selesai' ? 'disabled' : '' }} />
                            ...
                            <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white text-right">
                                <span><strong></strong> {{ number_format($subtotals[$item->id] ?? 0, 0, ',', '.') }}</span>
                            </td>
                            <td class="px-6 py-4 justify-items-end">
                                @if ($status !== 'Selesai')
                                    <a href="#" class="justify-items-end font-medium text-red-600 dark:text-red-500 hover:underline"
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex justify-end items-center">
                                            ...
                                            <button type="button" wire:click="returnToProduction({{ $rusak['id'] }}, {{ $rusak['unit_price'] }}, {{ $rusak['qty'] }})" class="text-blue-600 hover:underline">
