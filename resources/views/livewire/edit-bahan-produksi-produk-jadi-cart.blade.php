                ...
                <div class="space-y-3">
                    @foreach ($bahanKeluars as $keluar)
                        <x-kartu-pengajuan-bahan-keluar :keluar="$keluar"
                            judul="Pengajuan Bahan Produksi"
                            konteks="produksi ini" />
                    @endforeach
                </div>
            </div>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <input type="hidden" name="produksiProdukJadiDetails" value="{{ json_encode($this->getCartItemsForStorage()) }}">
                        ...
                        <td class="px-6 py-4 text-gray-900 dark:text-white text-center">
                            <div class="flex items-center">
                                @php
                                    ...
                                    // Bahan batangan stoknya tersimpan dalam cm. Tanpa pilihan
                                    // satuan, "2" akan terbaca 2 cm padahal maksudnya 2 batang.
                                    // Baris produk setengah jadi tidak punya panjang standar.
                                    $qtyKeyBaris = $bahanId ? 'b_' . $bahanId : null;
                                    $panjangStandarBaris = $qtyKeyBaris ? $this->panjangStandarUntuk($qtyKeyBaris) : null;
                                    $labelBatangBaris = $panjangStandarBaris ? $this->namaUnitUntuk($qtyKeyBaris) : null;
                                @endphp
                                ...
                                @if ($bahanId)
                                    ...
                                    @if ($panjangStandarBaris)
                                        <select wire:model="satuan.{{ $qtyKeyBaris }}" wire:change="updateSatuan('{{ $qtyKeyBaris }}')"
                            ...
                            @if ($panjangStandarBaris)
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                    keluar {{ number_format($this->qtyDasar($qtyKeyBaris), 0, ',', '.') }} cm
