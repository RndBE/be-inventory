<div>
    <div class="relative sm:rounded-lg pt-2">
        ...
        @if ($isFirstTimePengajuan && $bahanKeluars->isNotEmpty())
            @php
                $firstTimeMaterials = $bahanKeluars
        ...
        @if (!$isFirstTimePengajuan && $bahanKeluars->isNotEmpty())
            <div id="alert-additional-content-3"
                ...
                @foreach ($bahanKeluars as $keluar)
                    ...
                    @php
                        $materials = $keluar->bahanKeluarDetails
                            ->map(function ($detail) {
                                if ($detail->dataProduk) {
                                    // Jika produk, tambahkan serial number
                                    $nama = $detail->dataProduk->nama_bahan ?? 'Nama produk tidak tersedia';
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 w-0.5">Keterangan</th>
                ...
                <tbody>
                    @php
                        $grandTotal = 0;
                    @endphp
                    <input type="hidden" name="projekRndDetails"
                        ...
                        <tr wire:key= "projek-rnd-detail-{{ $id }}"
                            ...
                            <td class="px-6 py-4 text-gray-900 dark:text-white">
                                <div class="flex items-center">
                                    ...
                                    {{-- Input untuk Bahan --}}
                                    {{-- @if ($bahanId)
                                        <textarea wire:model.defer="keterangan_penanggungjawab.{{ $bahanId }}"
                                            class="bg-gray-50 w-48 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400
                                                dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            rows="2" name="projekRndDetails[{{ $bahanId }}][keterangan_penanggungjawab]" @if ($this->projekRndStatus === 'Selesai' || $this->projekRndStatus === 'Tidak dilanjutkan') disabled @endif></textarea>
                                    @endif --}}

                                    {{-- Input untuk Produk --}}
                                    {{-- @if ($produkId)
                                        <textarea wire:model.defer="keterangan_penanggungjawab.{{ $produkId }}"
                                            class="bg-gray-50 w-48 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400
                                                dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            ...
                            <td class="px-6 py-4 text-gray-900 dark:text-white text-center">
                                <div class="flex items-center">
                                    ...
                                    {{-- <input value="{{ old('qty.' . $id, $qty[$id] ?? 0) }}"
                                        ...
                                        class="bg-gray-50 w-20 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        placeholder="0" min="0" required
                                        @if (isset($detail['bahan_id'])) readonly @endif
                                        @if ($this->projekRndStatus === 'Selesai' || $this->projekRndStatus === 'Tidak dilanjutkan') disabled @endif /> --}}
                                    {{-- @php
                                        $bahanId = $detail['bahan_id'] ?? null;
                                        $produkId = $detail['produk_id'] ?? null;
                                    ...
                                    {{-- @if ($bahanId)
                                        <input value="{{ old('qty.' . $bahanId, $qty[$bahanId] ?? 0) }}"
                                            type="number" wire:model="qty.{{ $bahanId }}"
                                    ...
                                    {{-- @if ($produkId)
                                        <input value="{{ old('qty.' . $produkId, $qty[$produkId] ?? 0) }}"
                                            type="number" wire:model="qty.{{ $produkId }}"
                                ...
                                @foreach ($details as $d)
                                    ...
                                    <div class="flex flex-col space-y-2">
                                        <div class="flex justify-end items-center">
                                            <p>{{ $d['qty'] }} x
                                                {{ number_format($d['unit_price'], 0, ',', '.') }}</p>
                                            ...
                                            @if ($projekRndStatus !== 'Selesai' && $projekRndStatus !== 'Tidak dilanjutkan')
                                                ...
                                                @if ($bahanId)
                                                    <button
                                                        wire:click="decreaseQuantityPerPrice('bahan', {{ $bahanId }}, {{ $d['unit_price'] }})"
                                                    ...
                                                    <button
                                                        wire:click="returQuantityPerPrice('bahan', {{ $bahanId }}, {{ $d['unit_price'] }})"
                                                        class="inline-flex items-center justify-center p-1 text-sm font-medium h-6 w-6 text-gray-500 bg-white border border-gray-300 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                                                        type="button">
                                                ...
                                                @if ($produkId)
                                                    <button
                                                        wire:click="decreaseQuantityPerPrice('produk', {{ $produkId }}, {{ $d['unit_price'] }})"
                                                        class="inline-flex items-center justify-center p-1 text-sm font-medium h-6 w-6 text-gray-500 bg-white border border-gray-300 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                                                        type="button">
                                                        <svg class="w-3 h-3" aria-hidden="true"
                                                            xmlns="http://www.w3.org/2000/svg" fill="none"
                                                    ...
                                                    <button
                                                        wire:click="returQuantityPerPrice('produk', {{ $produkId }}, {{ $d['unit_price'] }})"
                                                        class="inline-flex items-center justify-center p-1 text-sm font-medium h-6 w-6 text-gray-500 bg-white border border-gray-300 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                                                        type="button">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24"
                                                            height="24" viewBox="0 0 24 24" fill="none"
            ...
            <div class=" border-gray-900/10">
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg pt-0">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <input type="hidden" name="bahanRusak"
                                value="{{ json_encode($this->getCartItemsForBahanRusak()) }}">
                            @foreach ($bahanRusak as $index => $rusak)
                                @php
                                    ...
                                    foreach ($projekRndDetails as $detail) {
                                        $match = false;
                                        if (
                                            isset($detail['bahan_id']) &&
                                            $detail['bahan_id'] == ($rusak['bahan_id'] ?? null)
                                        ) {
                                            $match = true;
                                        }
                                        if (
                                            isset($detail['produk_id']) &&
                                            $detail['produk_id'] == ($rusak['produk_id'] ?? null)
                                        ) {
                                            $match = true;
                                        }
                                        if ($match) {
                                ...
                                <tr
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        {{ number_format(
                                            round(floatval($rusak['unit_price'] ?? 0) * floatval(str_replace(',', '.', $rusak['qty'] ?? 0))),
                                            0,
                                            ',',
                                            '.',
                                        ) }}
            ...
            <div class=" border-gray-900/10">
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg pt-0">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <input type="hidden" name="bahanRetur"
                                value="{{ json_encode($this->getCartItemsForBahanRetur()) }}">
                            @foreach ($bahanRetur as $index => $retur)
                                @php
                                    ...
                                    foreach ($projekRndDetails as $detail) {
                                        ...
                                        if (
                                            isset($detail['bahan_id']) &&
                                            $detail['bahan_id'] == ($retur['bahan_id'] ?? null)
                                        ) {
                                            $match = true;
                                        }
                                        if (
                                            isset($detail['produk_id']) &&
                                            $detail['produk_id'] == ($retur['produk_id'] ?? null)
                                        ) {
                                            $match = true;
                                        }
                                        if ($match) {
                                ...
                                <tr
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        {{ number_format(
                                            round(floatval($retur['unit_price'] ?? 0) * floatval(str_replace(',', '.', $retur['qty'] ?? 0))),
                                            0,
                                            ',',
                                            '.',
                                        ) }}
