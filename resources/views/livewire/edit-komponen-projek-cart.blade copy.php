                        ...
                        <td class="px-6 py-4 text-gray-900 dark:text-white text-center">
                            <div class="flex items-center">
                                @php
                                    ...
                                    // pilih key: cart_key dulu, kalau nggak ada fallback ke salah satu id
                                    $inputKey = $detail['cart_key'] ?? $bahanId ?? $produkId ?? $produkJadiId;
                                    // type untuk keperluan method (bahan/produk/produk_jadi)
                                    if (isset($detail['cart_key'])) {
                                        // item baru — kita masih butuh type untuk beberapa logic
                                        $type = $bahanId ? 'bahan' : ($produkId ? 'produk' : 'produk_jadi');
                                    } else {
                                        // legacy item (no cart_key) — tentukan type berdasarkan id yang ada
                                        $type = $bahanId ? 'bahan' : ($produkId ? 'produk' : 'produk_jadi');
                                    }
                                @endphp
                                <input
                                    value="{{ old('qty.'.$inputKey, $qty[$inputKey] ?? 0) }}"
                                    type="number"
                                    wire:model="qty.{{ $inputKey }}"
                                    wire:keyup="updateQuantity('{{ $type }}', '{{ $inputKey }}')"
                                ...
                                {{-- @if ($produkJadiId)
                                    <input value="{{ old('qty.'.$produkJadiId, $qty[$produkJadiId] ?? 0) }}"
                                        type="number"
                                        wire:model="qty.{{ $produkJadiId }}"
                                        wire:keyup="updateQuantity('produk_jadi', {{ $produkJadiId }})"
                                        class="bg-gray-50 w-20 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        placeholder="0" min="0" required
                                        @if($this->produksiStatus === 'Selesai') disabled @endif />
                                @endif --}}
                                ...
                                <div class="flex flex-col space-y-2">
                                    <div class="flex justify-end items-center">
                                        ...
                                        @if($produksiStatus !== 'Selesai')
                                            ...
                                            {{-- @if ($produkJadiId)
                                                <button wire:click="decreaseQuantityPerPrice('produk_jadi', {{ $produkJadiId }}, {{ $d['unit_price'] }})"
                                                    class="inline-flex items-center justify-center p-1 text-sm font-medium h-6 w-6 text-gray-500 bg-white border border-gray-300 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                                                    type="button">
                                                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 2">
                                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 1h16"/>
                                                ...
                                                <button wire:click="returQuantityPerPrice('produk_jadi', {{ $produkJadiId }}, {{ $d['unit_price'] }})"
                                                    class="inline-flex items-center justify-center p-1 text-sm font-medium h-6 w-6 text-gray-500 bg-white border border-gray-300 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                                                    type="button">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-arrow-back">
                                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                        ...
                        <td class="px-6 py-4 flex justify-center items-center">
                            ...
                            @endphp
                                {{-- <a href="#" class="font-medium text-red-600 dark:text-red-500 hover:underline"
                                    wire:click.prevent="removeItem('{{ $detail['bahan_id'] ?? $detail['produk_id'] ?? $detail['produk_jadis_id'] }}')">
