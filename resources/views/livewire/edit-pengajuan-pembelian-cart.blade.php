                            ...
                            <tr
                                ...
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white text-right">
                                    @if ($editingItemId === $detail['bahan']->id)
                                        <input autofocus wire:model="unit_price_raw.{{ $detail['bahan']->id }}"
                                            type="text"
                                            class="w-full bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg
                                                focus:ring-blue-300 focus:border-blue-300 block px-2.5 py-1
                                                dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400
                                                dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 text-right
                                                cursor-not-allowed"
                                            placeholder="0" wire:blur="formatToRupiahPrice({{ $detail['bahan']->id }})"
                                            readonly />
                                    @else
                                ...
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white text-right">
                                    @if ($editingItemId === $field)
                                        <input autofocus wire:model="{{ $field }}_raw" type="text"
                                            ...
                                            placeholder="0" wire:blur="formatToRupiahPPN('{{ $field }}')"
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @php
                            ...
                            $biayaTambahanUSD =
                                ($shipping_cost_usd ?? 0) + ($full_amount_fee_usd ?? 0) + ($value_today_fee_usd ?? 0);
                        @endphp
                                ...
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white text-right">
                                    @if ($editingItemId === 'usd_' . $detail['bahan']->id)
                                        <input autofocus wire:model="unit_price_usd_raw.{{ $detail['bahan']->id }}"
                                            type="number"
                                            class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 text-right"
                                            placeholder="0" wire:blur="formatToUSDPrice({{ $detail['bahan']->id }})"
                                            @if ($status_finance === 'Disetujui') disabled @endif />
                                    @else
                                        @if ($status_finance !== 'Disetujui')
                                            <span>
                            ...
                            <tr
                                ...
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white text-right">
                                    ...
                                    @if ($editingItemBahan === $namaBahan)
                                        <input autofocus wire:model="unit_price_raw.{{ $namaBahan }}"
                                            type="text"
                                            class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 text-right"
                                            placeholder="0"
                                    ...
                                    @else
                                        @if ($status_finance !== 'Disetujui')
                                            <span>
                                                {{ number_format($unit_price_aset[$namaBahan] ?? ($detail['details']['unit_price'] ?? 0), 2, ',', '.') }}
                                            </span>
                                        @else
                                            <span>
                                                {{ number_format($unit_price_aset[$namaBahan] ?? ($detail['details']['unit_price'] ?? 0), 2, ',', '.') }}
                                ...
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                                    <div class="flex justify-right items-right">
                                        <textarea wire:model="keterangan_pembayaran.{{ $namaBahan }}"
        ...
        @if ($jenis_pengajuan === 'Pembelian Aset Impor')
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        {{-- Similar structure as above for Pembelian Aset Impor --}}
                        @php
                            $grandTotal = 0;
                            $grandTotalUSD = 0;
                            $biayaTambahan = ($shipping_cost ?? 0) + ($full_amount_fee ?? 0) + ($value_today_fee ?? 0);
