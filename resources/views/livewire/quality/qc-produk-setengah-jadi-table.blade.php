<div class="grid grid-cols-12 gap-6">
                            ...
                            <div class="flex justify-start items-start space-x-4">
                                ...
                                @can('addgudang-qc-produk-setengahjadi')
                                    @if(!$item->tanggal_masuk_gudang && $item->qc1)
                                        <button
                                            x-data wire:ignore
                                            x-on:click="$dispatch('open-gudang-modal', { id: {{ $item->id }}, jenis_sn: '{{ strtolower($item->jenis_sn) }}', kodeList: '{{ $item->kode_list }}' })"
    ...
    <div x-data="{ open: false, id: null, serial: '', jenis_sn: '', kodeList: '' }"
        x-on:open-gudang-modal.window="open = true; id = $event.detail.id; jenis_sn = $event.detail.jenis_sn;kodeList = $event.detail.kodeList;"
        x-show="open"
        class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50"
        style="display: none;"
        >
