                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @php
                            $grandTotal = 0;
                            $newGrandTotal = 0;
                            $biayaTambahan = ($ppn ?? 0) + ($ongkir ?? 0) + ($asuransi ?? 0) + ($layanan ?? 0) + ($jasa_aplikasi ?? 0);
                        @endphp
                        @foreach ($pembelianBahanDetails as $detail)
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    @else
                                        <span class="cursor-pointer" wire:click="editItemPrice({{ $detail['bahan']->id }})">
                                            {{ number_format(
                                                ...
                                                ($new_detail['new_details']['new_unit_price'] ?? 0), 2, ',', '.'
                                            ) }}
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white text-right">
                                    ...
                                    @else
                                        <span class="line-through text-red-500">{{ number_format($subTotal, 2, ',', '.') }}</span>
                                        <span class="font-semibold ml-2">{{ number_format($newSubTotal, 2, ',', '.') }}</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white text-right">
                                ...
                                $totalWithExtras = $grandTotal + $ppn + $ongkir + $asuransi + $layanan + $jasa_aplikasi;
                            @endphp
                                ...
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white text-right">
                                    ...
                                    @else
                                        <span class="cursor-pointer" wire:click="editItemPriceUSD('idr',{{ $detail['bahan']->id }})">
                                            {{ number_format(
                                                $new_unit_price[$detail['bahan']->id] ??
                                                ($detail['details']['new_unit_price'] ?? 0), 2, ',', '.'
                                            ) }}
                                        </span>
                                    @endif
                                </td>
