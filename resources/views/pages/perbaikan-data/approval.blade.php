...
<div x-data="{ isOpen: @entangle('isApproveModalOpen') }"
    x-show="isOpen"
    ...
    <div class="relative p-4 w-full max-w-md max-h-full"
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <div class="pt-0 p-5">
                <form class="formeditdata space-y-6" method="post" action="{{ route('perbaikan-data.updateApproval', (int)$id_perbaikan_data) }}">
                    @csrf
                    {{ method_field('PUT') }}
                    <input type="hidden" name="page" value="{{ $currentPage }}">
                    <div x-data="{ status: '{{ $status }}' }">
                        <label for="status" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status</label>
                        <select x-model="status" name="status" id="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                            <option value="" disabled>Pilih Status</option>
                            <option value="Diajukan " {{ $status === 'Diajukan ' ? 'selected' : '' }}>Diajukan </option>
                            <option value="Dalam Pemeriksaan" {{ $status === 'Dalam Pemeriksaan' ? 'selected' : '' }}>Dalam Pemeriksaan</option>
                            <option value="Disetujui" {{ $status === 'Disetujui' ? 'selected' : '' }}>Disetujui</option>
                            <option value="Ditolak" {{ $status === 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                            <option value="Sedang Diperbaiki" {{ $status === 'Sedang Diperbaiki' ? 'selected' : '' }}>Sedang Diperbaiki</option>
                            <option value="Selesai" {{ $status === 'Selesai' ? 'selected' : '' }}>Selesai</option>
                        </select>

                        <!-- Input Catatan jika Ditolak -->
                        <div x-show="status === 'Ditolak'" class="mt-4">
                    ...
                    @include('components.approval-kendala-field', [
                        'value' => \App\Models\ApprovalKendala::getKendala('perbaikan_data', (int) $id_perbaikan_data, 'Approval')
                    ])
