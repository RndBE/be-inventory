                        ...
                        <table class="min-w-full divide-y divide-slate-500">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b border-slate-200">
                                    ...
                                    <td class="hidden px-3 py-4 text-sm text-right text-slate-500 sm:table-cell"><span class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-green-400 border border-green-400">{{ $detail->panjangStandarEfektif() ? $detail->formatQty($detail->qty) : $detail->qty . ' ' . ($detail->dataBahan->dataUnit->nama ?? 'N/A') }}</span></td>
                                    {{-- Untuk bahan batangan, unit_price tersimpan per cm. Yang
                                         ditampilkan harga per batang, karena itulah angka yang ada
                                         di invoice supplier. --}}
                                    <td class="hidden px-3 py-4 text-sm text-right text-slate-500 sm:table-cell">Rp. {{ number_format($detail->panjangStandarEfektif() ? $detail->hargaPerBatang() : $detail->unit_price, 2) }}</td>
